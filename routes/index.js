...
app.map({
  '/api': {
	...
	'/s': { //search
	  '/:ss': { // search string
	    get: api.e.eid.s.ss.get // search for appeals, lists, users, officers, conversations etc, universal search. This will be used to search for everything. 
	...
	'/c': { // conversations
	  get: api.e.eid.c.get, // for getting all user's conversations via their token auth, active and archived are only denoted by the conversation's property in the db
	  post: api.e.eid.c.post, // for starting a new conversation
	  put: api.e.eid.c.put, // for editing conversations in bulk, whatever it may be
	  delete: api.e.eid.c.delete, // for deleting or archiving conversations in bulk, two step auth
	  '/:cid': { // conversation id
	    get: api.e.eid.c.cid.get, // get the latest 50 messages, this is different from push systems
	    post: api.e.eid.c.cid.post, // post a msg to conversation, with token auth, checks if poster is valid participant, resurrect archived conversation
	    put: api.e.eid.c.cid.put, // edits and deletes posts, add and remove participants, change conversation options
	    delete: api.e.eid.c.cid.delete, // archives the conversation and it no longer is active
	  }
	},
	'/u': { // users
	  ...
	  delete: api.e.eid.u.delete, // admin use, batch deleting users, JSON arguments
	  '/:uid': { // user id
