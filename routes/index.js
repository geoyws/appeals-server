...
app.map({
  '/e': { // entity
    ...
    delete: api.e.delete,
    '/:eid': { // entity id
      '/p': { // public (for public functions)
	'/s': { //search
	  '/:ss': { // search string
	    get: api.e.eid.p.s.ss.get // search for appeals, lists, users, officers, conversations etc, universal search
	  }
	}
      },
      '/u': { // users
	...
	'/:uid': { // user id
	  ...
	  '/c': { // conversations
		...
		'/:ss': { // search string
		  ...
		  // want recent appeals? postman http://appeals.io/api/e/eid/p/s/:ss this is the public search api
		}
	      }
	    }
	  },
	  '/a': { // appeals
	    get: api.e.eid.u.uid.a.get, // gets all of user's submitted appeals
	    post: api.e.eid.u.uid.a.post, // post new appeal by user
	    put: api.e.eid.u.uid.a.put, // edits appeals
